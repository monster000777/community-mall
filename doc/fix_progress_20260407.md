...
### 🔵 Bug 4：团购模块立即参团确认界面在暗色模式下未适配的问题（前端）
- **文件**：`frontend/src/views/GroupActivityDetail.vue`
- **问题描述**：在进行团购商品下单（点击“立即参团”）弹出的确认模态框（Modal）中，底部的“商品单价、购买数量、应付总额”结算汇总区域以及收货地址和输入框备注说明的文字，颜色使用了硬编码（如 `#f5f5f5`, `#666`, `#999`）。这导致在深色/暗黑模式下，文本由于在错误的底色上而无法阅读或者对比度差，界面整体也显得不自适应。
- **修复内容**：移除了硬编码颜色，将结算容器的 `background` 及内部边框 `border` 调整为自适应跟随应用层全局风格的 CSS 变量（`var(--bg-body)`、`var(--border-color)`），并将硬编码灰色文字改为 `var(--text-primary / secondary / tertiary)`，使得深色模式和浅色模式下该确认窗口都能完美自适应兼容。
