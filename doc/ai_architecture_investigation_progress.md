...
## 9. 部署指南与项目文档优化 (2026-05-30)
- **README 优化**：已对根目录下的 [README.md](file:///e:/community-mall/community-mall-master/README.md) 进行了全面重写与细化。
- **完善部署全流程**：新增并详述了 Redis 的多端环境安装启动步骤、大模型 AI 智能模块的两套部署配置方案（本地私密 `application-local.yml` 以及系统环境变量注入）、后端/前端编译及热启动的完整流转命令、以及新增的功能模块和接口排错指南。这使得新检出项目的开发者能够无障碍快速跑通包含 AI 部分的整套系统。

## 10. 关于 GitHub Actions CI/CD 流程的规划讨论 (2026-05-30)
- 评估了本项目引入持续集成与持续部署（CI/CD）的必要性。
- 确定了若要实现，核心在于：1) 前后端编译正确性自动化检查；2) 借助 `pom.xml` 插件完全剥离 `application-local.yml` 进行安全打包；3) 配合容器化部署将 AI 敏感密钥完全以系统环境变量形式注入，防止密钥外泄风险。
